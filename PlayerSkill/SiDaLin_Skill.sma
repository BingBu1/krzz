    ...
    new sTrail , g_Explosion , Float:NextSendBoom
    new bool:IsSkillUsed
    public plugin_init(){
        new plid = register_plugin("角色技能-斯大林" , "1.0" , "Bing")

        register_event("HLTV", "event_roundstart", "a", "1=0", "2=0")

        RegPlayerSkill(plid , "SiDaLin_Skill" , "bing_sidalin" , 600.0)
    }
    ...
    public SiDaLin_Skill(id){
        ...
        m_print_color(0 , "!g[冰布提示]!t%s释放了斯大林技能,集束轰炸。" , username)
        CreateBoomThink(id)
        IsSkillUsed = true
    }

     public FindNearNpc(Apaqi){
        new Float:fOrigin[3] ,Float:Origin2D[3]
        ...
        Origin2D[2] = 0.0
        while ((ent = rg_find_ent_by_class(ent , "SiDaLin_SkillThink_ent" , true)) > 0){
    ...
    public SkillCreateBoomThink(ent){
    ...
    public tk_Think(this){
        ...
        while((ent = find_ent_in_sphere(ent , fOrigin , 55.0)) > 0){
            if(ent != this && get_entvar(ent , var_flags) & FL_MONSTER){
                new Float:org[3]
                get_entvar(ent, var_origin, org)
                if(NextSendBoom < get_gametime()){
                    NextSendBoom = get_gametime() + 0.5
                    MakeBoom(org)  // 生成爆炸效果
                }
                // 标记删除
