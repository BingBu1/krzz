...
#define M32_DAMAGE 150.0

#define MODEL_V "models/v_M32.mdl"
#define MODEL_P "models/p_M32.mdl"
#define MODEL_W "models/w_M32.mdl"
...
public plugin_init()
{
	new plid = register_plugin("Grenade Launcher M32", "3.0", "Remade by Dias")
...
public event_roundstart(){
    arrayset(g_has_M32, 0 ,sizeof g_has_M32)
}
		...
		if(equal(classname, "M32_grenade"))
		{
			...
			while((a = find_ent_in_sphere(a,originF,300.0)) != 0) {
				if(a!=owner&& a!=ptr&&pev(a,pev_takedamage)!=DAMAGE_NO) {
					new Float:targetOrgin[3]
					pev(a, pev_origin, targetOrgin)
					new Float:dis = vector_distance(targetOrgin,originF)
					if(dis > 300.0) continue
					new Float:DamageNew = M32_DAMAGE * (1.0 - (dis / 300.0))
					ExecuteHamB(Ham_TakeDamage, a ,owner ,owner,  DamageNew, DMG_BULLET)
				}
