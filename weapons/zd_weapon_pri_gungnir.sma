...
// Data Config
#define P_GUNGNIR "models/ZombieDarkness/Weapon/Pri/p_gungnira.mdl"
#define P_GUNGNIR2 "models/ZombieDarkness/Weapon/Pri/p_gungnirb.mdl"
#define V_GUNGNIR "models/ZombieDarkness/Weapon/Pri/v_gungnir.mdl"
#define W_GUNGNIR "models/ZombieDarkness/Weapon/Pri/w_gungnir.mdl"

#define MF_W "sprites/ZombieDarkness/muzzleflash81.spr"
...
new const SOUND_FIRE[][] = 
{
	...
	"weapons/gungnir_charge_loop.wav",
	"weapons/gungnir_charge_shoot2.wav",
	"weapons/gungnir_draw.wav",
	"weapons/gungnir_idle.wav",
	"weapons/gungnir_reload.wav",
	"weapons/gungnir_shoot_b_charge.wav"
}

new const EXP_MODELS[][] = 
{
	"sprites/ZombieDarkness/ef_gungnir_aexplo.spr",
	"sprites/ZombieDarkness/ef_gungnir_bexplo.spr",
	"sprites/ZombieDarkness/ef_gungnir_chargeexplo.spr",
	"sprites/ZombieDarkness/ef_gungnir_lightline1.spr",
	"sprites/ZombieDarkness/ef_gungnir_lightline2.spr",
	"sprites/ZombieDarkness/ef_gungnir_missile.spr",
	"sprites/ZombieDarkness/ef_gungnir_xbeam.spr",
	
	"models/ZombieDarkness/Weapon/Pri/gungnir_missile.mdl"
}
...
public plugin_init()
{
	...
	BulidCrashGunWeapon("断域雷魂" , W_GUNGNIR , "GiveWpn", plid)
	// g_Dprd = zd_weapon_register("断域雷魂", WEAPON_PRIMARY, CSW_GUNGNIR, 24000, 20)
}
...
public zd_weapon_remove(id, WeaponType)
{
	// if(WeaponType == WEAPON_PRIMARY && Get_BitVar(g_Had_Base, id)) Remove_Base(id)
}

public zd_weapon_addammo(id, ItemID)
{
	// if(ItemID == g_Dprd && Get_BitVar(g_Had_Base, id)) cs_set_user_bpammo(id, CSW_GUNGNIR, BPAMMO)
}
...
public plugin_precache()
{
	...
	precache_generic("sprites/ZombieDarkness/640hud36.spr")
	precache_generic("sprites/ZombieDarkness/640hud44.spr")
	// precache_generic("sprites/ZombieDarkness/640hud185.spr")
...
public Get_Base(id)
{
	...
	rg_give_item(id, weapon_gungnir, GT_DROP_AND_REPLACE)
...
public WE_GUNGNIR(id,iEnt,iClip, bpammo,iButton)
{
	...
	if(!(iButton & IN_ATTACK2) && 0 < iCharge < 3)
	{
		...
		// get_position(id, 20.0,0.0,0.0,vOrigin)
		pev(id, pev_v_angle, vVAngles);
	
		Stock_Get_Aiming(id, vecEnd2)

		Stock_GetSpeedVector(vOrigin, vecEnd2, iMax?2000.0:1500.0, vVec)
		...
		set_pdata_float(iEnt, 46, iMax?3.13:SPEED2, 4)
		set_pdata_float(iEnt, 48, iMax?3.5:SPEED2+0.25, 4)
		...
		if(iState == 1)
		{
			...
			for(k = 0 ; k < 5 ; k++){
				while((pEntity = engfunc(EngFunc_FindEntityInSphere, pEntity, fOrigin, ELECTRO_RANGE)) != 0){
					...
					get_entvar(pEntity , var_origin ,tempOrigin)
					TmpDis = get_distance_f(fOrigin, tempOrigin)
					if(TmpDis > ELECTRO_RANGE)
					 	continue
					if(NearDis == 0.0 || TmpDis < NearDis){
						NearDis = TmpDis
						g_iVic[k] = pEntity
					}
				}
...
{
	if(iCheck && get_entvar(id, var_weaponanim) == anim)
		return;
...
stock Stock_Get_Postion(id,Float:forw, Float:right, Float:up, Float:vStart[])
{
	...
	xs_vec_add(vOrigin,vUp,vOrigin)
	get_entvar(id, var_v_angle, vAngle) // if normal entity ,use pev_angles
