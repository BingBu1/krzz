...
Float:g_thunderbolt_zoomdelay[33], Float:g_thunderbolt_zoomdelay2[33], Float:g_thunderbolt_zoomdelay3[33], Float:g_thunderbolt_zoomdelay4[33]
...
public ChangHadWaepon(id, bool:ishas){
	g_had_thunderbolt[id] = ishas
}
...
public Event_CurWeapon(id)
{
	...
	if(get_user_weapon(id) == CSW_THUNDERBOLT && g_had_thunderbolt[id])
	{
		...
		set_player_light(id, DEFAULT_MAP_LIGHT)
		set_player_screenfade(id, 0, 0, 0, 127, 255, 255, 0)
	}
...
public fw_CmdStart(id, uc_handle, seed)
{
		...
		if(get_gametime() - ZOOM_DELAY > g_thunderbolt_zoomdelay[id])
		{
			if(get_pdata_float(id, 83, 5) <= 0.0)
			{
				if(cs_get_user_zoom(id) == 1)
				{
					...
					set_player_light(id, ZOOM_LIGHT)
					set_player_screenfade(.pPlayer = id,.sDuration = (1 << 12),.sHoldTime = 0,.sFlags = 0x0004,.r = 127,.g = 225,.b = 255,.a = 70)
				} else {
					cs_set_user_zoom(id, 1, 1)
					set_player_light(id, DEFAULT_MAP_LIGHT)
					set_player_screenfade(id, 0, 0, 0, 127, 255, 255, 0)
	...
	if(get_gametime() - ZOOM_DELAY2 > g_thunderbolt_zoomdelay2[id])
	{
		...
		if(get_gametime() - (ZOOM_DELAY2 + 0.25) > g_thunderbolt_zoomdelay4[id])
		{
			if(cs_get_user_zoom(id) != 1)
			{
				set_player_light(id, ZOOM_LIGHT)
			...
			g_thunderbolt_zoomdelay4[id] = get_gametime()
		}
...
//this, idattacker, Float:damage, Float:direction[3], tracehandle, damagebits
public fw_TraceAttack(ent, attacker, Float:Damage, Float:fDir[3], ptr, iDamageType)
{
	...
	// SetHookChainArg(3, ATYPE_FLOAT, DAMAGE)
